...
def nuc_norm_recovery(data, var, lambd):
    'fast proximal gradient method to solve argmin_{X} (1 / 2*var) \|X - data\| + lambd * \|X\|_{nuc}'
...
def nuc_norm_recovery(data, var, lambd, reltol, tol=None, num_sweeps=None):
    'fast proximal gradient method to solve argmin_{X} (1 / 2*var) \|X - data\| + lambd * \|X\|_{nuc}'
